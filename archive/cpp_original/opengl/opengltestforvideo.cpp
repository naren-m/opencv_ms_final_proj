...
glMatrixMode(GL_MODELVIEW); 
  ...
  glTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0,  0, 0,  onImage->width, onImage->height, GL_BGR, GL_UNSIGNED_BYTE, onImage->imageData);
	  


  if(RectangleFoundFlag ==1)
    {


  int firstMinx,secondMinx,firstMiny,secondMiny;
	   firstMinx = 0;// onPoint[0]->x;
	   secondMinx =0;// onPoint[0]->x;
 firstMiny = 0;// onPoint[0]->x;
	   secondMiny =2;// onPoint[0]->x;


	   for(int j = 0;j<4;j++)
     {
       if((onPoint[j]->x) < (onPoint[firstMinx]->x) )
	 firstMinx = j;

	
 if((onPoint[j]->y) < (onPoint[firstMiny]->y) )
	 firstMiny = j;
   ...
   for(int j = 0;j<4;j++)
     {	 
       if((onPoint[j]->x <  onPoint[secondMinx]->x) && (j != firstMinx))
	 secondMinx = j; 

 if((onPoint[j]->y <  onPoint[secondMiny]->y) && (j != firstMiny))
	 secondMiny = j; 
     }	 

   // printf("x1 - %d , y1 - %d ,x2 - %d , x2 - %d \n x min - %d x2 min - %d \n" , onPoint[0]->x,onPoint[1]->x , onPoint[2]->x,onPoint[3]->x,onPoint[firstMin]->x,onPoint[secondMin]->x);

   //printf("x1 - %d , y1 - %d ,x2 - %d , y2 - %d \n" , onPoint[firstMinx]->x,onPoint[firstMiny]->y , onPoint[secondMinx]->x,onPoint[secondMiny]->y);
	   
   int topl,topr,botl;

   if(firstMinx == firstMiny)
     topl = firstMinx;
   else if(firstMinx == secondMiny)
     topl = firstMinx;
   else if(secondMinx == firstMiny)
     topl = secondMinx;
   else if (secondMinx == secondMiny)
     topl = secondMinx;

   // finding right corner
   topr = topl+1;
   if(topr == 4)
     topr = 0;
 ...
 botl =  topl-1;
   ...
   int temptr , tempbl;
   //  temp = topr;
   if(onPoint[botl]->x > onPoint[topr]->x)
     {
       temptr = botl; 
       tempbl = topr;
     }
   ...
   if((topl == 0 || topl == 1 || topl == 2 ) && (topr == 0 || topr == 1 || topr == 2 ) && ( botl == 0 || botl == 1 || botl ==2))
     botr = 3;

      if((topl == 3 || topl == 1 || topl == 2 ) && (topr == 3 || topr == 1 || topr == 2 ) && ( botl == 3 || botl == 1 || botl ==2))
     botr = 0 ;

      if((topl == 0 || topl == 3 || topl == 2 ) && (topr == 0 || topr == 3 || topr == 2 ) && ( botl == 0 || botl == 3 || botl ==2))
     botr = 1;

      if((topl == 0 || topl == 1 || topl == 3 ) && (topr == 0 || topr == 1 || topr == 3 ) && ( botl == 0 || botl == 1 || botl ==3))
     botr = 2;

   

printf("x%d - %d , y%d - %d ,x%d - %d , y%d - %d \n point is - %d \n" , 
	  firstMinx, onPoint[firstMinx]->x,
	  firstMiny, onPoint[firstMiny]->y,
	  secondMinx,onPoint[secondMinx]->x,
       secondMiny,onPoint[secondMiny]->y,topl);

 printf ("topr -  %d , botl -  %d , topl -  %d , botr -  %d\n",topr,botl,topl,botr);
 ...
 /*   glTexCoord2i(0,0);                            glVertex3f(onPoint[0]->x,onPoint[0]->y,1.5);           
	   ...
	   glTexCoord2i(0,onImage->height);               glVertex3f(onPoint[3]->x,onPoint[3]->y,1.5);*/
 	   ...
 	   glTexCoord2i(0,0);                            glVertex3f(onPoint[topl]->x,onPoint[topl]->y,1.5);           
	   glTexCoord2i(onImage->width,0);              glVertex3f(onPoint[topr]->x,onPoint[topr]->y,1.5); 
	   glTexCoord2i(onImage->width,onImage->height); glVertex3f(onPoint[botr]->x,onPoint[botr]->y,1.5);
	   glTexCoord2i(0,onImage->height);               glVertex3f(onPoint[botl]->x,onPoint[botl]->y,1.5); 
...
{ 
   glutInit(&argc,argv); 
    ...
    IplImage *camFrame = 0;;
     ...
     CvCapture *captureVideo = cvCaptureFromFile("test.avi");

     //  onImage = cvLoadImage("1.png");
  onImage = cvLoadImage("ram.jpg");
    // always check 
    // always check 
    if( !capture ) 
{ 
...
onImage = cvQueryFrame(captureVideo);
