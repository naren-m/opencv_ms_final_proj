...
setup: venv ## Set up the development environment
	...
	$(VENV_PIP) install -r pysrc/requirements.txt
	@echo "$(GREEN)Setup complete!$(NC)"
	@echo "$(YELLOW)To activate virtual environment: source $(VENV_DIR)/bin/activate$(NC)"

.PHONY: setup-dev
setup-dev: setup ## Set up development environment with additional tools
	@echo "$(GREEN)Installing development tools...$(NC)"
	$(VENV_PIP) install jupyter black flake8 pytest pytest-cov
...
docker-run: ## Run Docker container with camera access
	...
	@echo "$(YELLOW)Note: GUI applications may not work on macOS without X11 server$(NC)"
	docker run --rm \
		--name $(CONTAINER_NAME) \
		-v $(PWD)/pysrc:/app \
...
run: ## Run the main object tracking application
	...
	cd pysrc && ../$(VENV_PYTHON) object_tracking.py
...
run-face-detection: ## Run face detection application
	...
	cd pysrc && ../$(VENV_PYTHON) task3/face_detection.py
...
run-edge-detection: ## Run Canny edge detection application
	...
	cd pysrc && ../$(VENV_PYTHON) task4/canny_edge_detection.py
...
run-shape-detection: ## Run shape detection application
	...
	cd pysrc && ../$(VENV_PYTHON) task4/square_detection.py

.PHONY: run-histogram
run-histogram: ## Run real-time histogram application
	@echo "$(GREEN)Running histogram application...$(NC)"
	cd pysrc && ../$(VENV_PYTHON) task2/histogram_realtime.py

# Testing and Quality
.PHONY: test
test: ## Run tests
	@echo "$(GREEN)Running tests...$(NC)"
	cd pysrc && ../$(VENV_PYTHON) -m pytest -v

.PHONY: lint
lint: ## Run code linting
	@echo "$(GREEN)Running code linting...$(NC)"
	cd pysrc && ../$(VENV_PYTHON) -m flake8 . --count --select=E9,F63,F7,F82 --show-source --statistics

.PHONY: format
format: ## Format code with black
	@echo "$(GREEN)Formatting code...$(NC)"
	cd pysrc && ../$(VENV_PYTHON) -m black .
...
info: ## Show project information
	...
	@echo "OpenCV Version: $$(cd pysrc && ../$(VENV_PYTHON) -c 'import cv2; print(cv2.__version__)' 2>/dev/null || echo 'Not installed')"
...
test-camera: ## Test camera access
	...
	cd pysrc && ../$(VENV_PYTHON) -c "import cv2; cap = cv2.VideoCapture(0); print('Camera available:', cap.isOpened()); cap.release()" || \
	(echo "$(RED)Camera access failed!$(NC)" && \
	 echo "$(YELLOW)On macOS, you need to grant camera permissions:$(NC)" && \
	 echo "$(YELLOW)1. Go to System Preferences > Security & Privacy > Camera$(NC)" && \
	 echo "$(YELLOW)2. Grant permission to Terminal or your IDE$(NC)" && \
	 echo "$(YELLOW)3. Restart your terminal and try again$(NC)")
